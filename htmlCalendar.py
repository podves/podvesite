...
# Configuration
#=========================================================================
        # title: font family (CSS-spec, e.g. "Arial, Helvetica")
        ...
        # heading: font family (CSS-spec, e.g. "Arial, Helvetica")
        ...
        # days: font family (CSS-spec, e.g. "Arial, Helvetica")
        ...
        # weeks: font family (CSS-spec, e.g. "Arial, Helvetica")
        ...
        # hilight color (works only in combination with link)
        self.hilightColor = '#FFFF00'

        # page to link to when day is clicked
        self.link = ''
        # link target frame or window, e.g. parent.myFrame
        self.linkTarget = ''
        # week start: 0 - 6 (0 = Saturday, 1 = Sunday, 2 = Monday ...)
        self.offset = 2
        # view week numbers: 1 = yes, 0 = no
        self.weekNumbers = 1

#-------------------------------------------------------------------------
# You should change these variables only if you want to translate them into your language:
#-------------------------------------------------------------------------
        # weekdays: must start with Saturday because January 1st of year 1 was
        # a Saturday
...
#=========================================================================
    ...
    def set_styles(self):
        ...
        if self.wFontFace:
            html += 'font-family: ' + self.wFontFace + '; '
        if self.wFontSize:
            html += 'font-size: ' + str(self.wFontSize) + 'px; '
        if self.wFontColor:
            html += 'color: ' + self.wFontColor + '; '
        if self.wBGColor:
            html += 'background-color: ' + self.wBGColor + '; '
    ...
    def table_head(self, content):
        ...
        for i in range(len(self.weekdays)):
            ...
            html += self.table_cell(
                wDay, 'cssHeading' + str(globals()['cal_ID']))
        if self.weekNumbers:
            html += self.table_cell(
                '&nbsp;', 'cssHeading' + str(globals()['cal_ID']))
        html += '</tr>'
        return html
    ...
    def viewEvent(self, start, end, color, title, link=''):
        ...
        if end < 1 or end > 31:
            return
        while start <= end:
            if not self.specDays.has_key(str(start)):
                self.specDays[str(start)] = []
    ...
    def viewEventEach(self, weekday, color, title, link=''):
        ...
        if weekday < 0 or weekday > 6:
            return
        if not self.specDays2.has_key(str(weekday)):
            self.specDays2[str(weekday)] = []
    ...
    def create(self):
        ...
        if self.year < 1 or self.year > 3999:
            ...
            html += '<td' + \
                (self.borderColor and ' bgcolor=' + self.borderColor) + '>'
            ...
            if self.weekNumbers or self.week:
            ...
            for i in range(self.__mDays[self.month - 1] + 1):
                for j, v in self.specDays2.items():
                    if self.get_weekday(self.year, days + i) == int(j) - self.offset + 1:
                        if not self.specDays.has_key(str(i)):
                            self.specDays[str(i)] = []
            ...
            while daycount <= stop:
                ...
                for i in range(len(self.weekdays)):
                    ...
                    if (daycount == 1 and i < start) or daycount > stop:
                        ...
                        if inThisMonth and daycount == curDay:
                            style = 'padding:0px;border:3px solid ' + \
                                self.tdBorderColor + ';'
                ...
                if self.weekNumbers:
                    ...
                    html += self.table_cell(
                        content, 'cssWeeks' + str(globals()['cal_ID']))
                    weekNr += 1
